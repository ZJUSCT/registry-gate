...
var pageNames = []string{"login", "dashboard", "issued", "usage", "error"}
...
func parseTemplates() (*templates, error) {
	...
	for _, name := range pageNames {
		...
		if err != nil {
			return nil, fmt.Errorf("portal: parse template %s: %w", name, err)
		}
		t.pages[name] = set
	}
	return t, nil
...
func (s *server) render(w http.ResponseWriter, status int, page string, data any) {
	...
	if !ok {
		http.Error(w, "portal: unknown page "+page, http.StatusInternalServerError)
		return
	}
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.WriteHeader(status)
	if err := t.ExecuteTemplate(w, "layout", data); err != nil {
		s.logger.Error("portal: render page", "page", page, "err", err)
	}
}
